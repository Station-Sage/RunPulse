...
현재의 `raw_raw_raw_source_payloads` 테이블은 이 raw-ingestion ledger 역할의 중심으로 사용하고, 필요 시 컬럼을 확장한다.
...
공통 core schema에 넣기에는 너무 소스 특화적이거나, 아직 안정성이 확인되지 않은 값은 `activity_detail_metrics` 같은 별도 계층에 저장한다.
...
4. `activity_summaries`, `daily_wellness`, `daily_fitness` 같은 core table에 쓴다.
5. `activity_detail_metrics`에 소스 특화 지표를 쓴다.
...
- analysis 쪽에서도 `activity_detail_metrics`를 활용하고 있다.
...
Runalyze의 특수 지표는 우선 `activity_detail_metrics`에 저장하고, 여러 기능에서 공통적으로 필요한 경우에만 core schema 반영을 검토한다.
...
- 소스 특화 지표는 `activity_detail_metrics` 같은 유연한 저장 계층으로 분리한다.
