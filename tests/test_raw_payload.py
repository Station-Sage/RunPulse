"""source_payloads 저장/병합 유틸리티 테스트 (v0.3 스키마)."""
...
@pytest.fixture()
def conn():
    """인메모리 SQLite DB — source_payloads + activity_summaries 포함."""
...
class TestStoreRawPayload:
    def test_insert_new(self, conn):
        ...
        row = conn.execute(
            "SELECT payload FROM source_payloads WHERE entity_id = '123'"
        ).fetchone()
        assert row is not None
        assert json.loads(row[0]) == {"key": "val"}

    def test_second_call_replaces_payload(self, conn):
        """두 번째 호출 시 payload 전체 교체 (key-level merge 없음)."""
        ...
        row = conn.execute(
            "SELECT payload FROM source_payloads WHERE entity_id = '1'"
        ).fetchone()
        data = json.loads(row[0])
        assert data["new_key"] == "new"
        assert data["common"] == "new_val"
        assert "old_key" not in data  # 전체 교체 — 기존 고유 키는 사라짐

    def test_update_replaces_when_changed(self, conn):
        """payload가 변경되면 전체 교체."""
        store_raw_payload(conn, "strava", "activity_summary", "A", {"hr": 140, "pace": 300})
        store_raw_payload(conn, "strava", "activity_summary", "A", {"hr": 145})
        data = json.loads(
            conn.execute("SELECT payload FROM source_payloads").fetchone()[0]
        )
        assert data["hr"] == 145
        assert "pace" not in data  # 전체 교체
    ...
    def test_activity_id_set_on_insert(self, conn):
        ...
        row = conn.execute("SELECT activity_id FROM source_payloads").fetchone()
        assert row[0] == 42
    ...
    def test_empty_payload_skipped(self, conn):
        ...
        row = conn.execute("SELECT COUNT(*) FROM source_payloads").fetchone()
        assert row[0] == 0
    ...
    def test_fetched_at_present_after_update(self, conn):
        """업데이트 후 fetched_at이 존재한다."""
        store_raw_payload(conn, "garmin", "activity", "T", {"v": 1})
        store_raw_payload(conn, "garmin", "activity", "T", {"v": 2})
        ts = conn.execute("SELECT fetched_at FROM source_payloads").fetchone()[0]
