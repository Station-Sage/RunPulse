...
서브에이전트 6개 + 스킬 6개로 역할을 분리한다.
architect 2개는 `claude --agent <name>` 세션 모드로 대화형 사용.
나머지 4개는 `@`-mention 서브에이전트로 대량 출력 격리 시 사용.
모든 서브에이전트는 Write/Edit 불가 — 보고서만 반환한다.
모든 코드/문서 수정은 메인 에이전트가 직접 수행한다.
상세: `.claude/AGENT-ROADMAP.md` 참조.
커밋 전 /pre-commit 스킬을 실행한다.
상세: .claude/AGENT-ROADMAP.md 참조.
