...
기존 `db_setup.py`의 현재 스키마를 기반으로, v0.3 설계서의 11개 파이프라인 테이블 (+ 1 뷰)과 하나씩 대조하며 최종 DDL을 확정하겠습니다.
...
여기가 가장 중요합니다. v0.3 설계서에서 ~38컬럼 (id 포함)으로 정의했는데, 이제 하나하나 확정합니다.

**컬럼 포함 기준 재확인**:

**기준 0 (최우선)**: "기본 데이터 vs 메트릭" — 센서 직접 측정값 또는 단순 산술 파생값만 activity_summaries에 둔다. 알고리즘 산출물(training_load, suffer_score, training_effect_*, normalized_power, calories 등)은 metric_store로. (architecture.md Part 2 참조)

기준 1 (보조): "활동 목록 페이지에서 한 행으로 보여줄 때 필요한가?" — distance, duration, pace, HR, cadence 등은 필수.
기준 2 (보조): "필터/정렬 조건으로 쓰이는가?" — activity_type, start_time, distance_m 등.
기준 3 (보조): "활동 카드(요약)에 즉시 표시되는가?" — running dynamics, HR, pace 등.
...
> **설계 변경 (2026-04-04)**: 초기 설계에서는 training_load, suffer_score,
> training_effect_aerobic/anaerobic, normalized_power, calories를
> activity_summaries에 포함했으나, "기본 데이터 vs 메트릭" 원칙(architecture.md Part 2) 적용 후
> 전부 metric_store로 이동. 이 6개는 알고리즘 산출물이므로 activity_summaries에 두지 않는다.
...
### `activity_summaries` 최종 DDL — 38컬럼 확정 (id 포함)
...
|------|------------|------------|------|
| 총 컬럼 수 | 80+ (대부분 미사용) | 38 (전부 용도 명확, 메트릭 6개 metric_store 이동) | 역할 분리 |
...
| `training_load` | activity_summaries | → metric_store | 알고리즘 산출물 |
| `suffer_score` | activity_summaries | → metric_store | 알고리즘 산출물 |
| `training_effect_*` | activity_summaries | → metric_store | 알고리즘 산출물 |
| `normalized_power` | activity_summaries | → metric_store | 알고리즘 산출물 |
| `calories` | activity_summaries | → metric_store | 알고리즘 산출물 |
...
**16컬럼 (id 포함)**. 대시보드 "오늘의 상태" 카드에 직접 표시되는 값만 포함합니다. sleep 상세(deep/light/rem/awake), stress 상세(high/medium/low duration), training readiness 상세, SpO2, 호흡수 등은 전부 `metric_store(scope_type='daily')`로 갑니다.
...
CREATE TABLE IF NOT EXISTS weather_cache (
    ...
    -- NOTE: latitude/longitude는 Python 단에서 소수점 2자리로 rounding 후 저장
);
```
...
총 **11개 파이프라인 테이블 + 5개 앱 테이블 = 16개 테이블 + 1개 뷰**. 기존 v0.2의 35+개에서 대폭 축소했으면서도 데이터는 하나도 잃지 않습니다.
...
def upsert_activity_summary(conn, data: dict) -> int:
    ...
    valid_columns = get_activity_summary_columns()  # 38개 컬럼명 set
...
# 소스별 race prediction → 통일 이름
assert canonicalize("raceTime5K", source="garmin") == ("race_pred_5k", "rp_prediction")
assert canonicalize("darp_5k_sec", source="runpulse") == ("race_pred_5k", "rp_prediction")
assert canonicalize("race_pred_5k_sec", source="intervals") == ("race_pred_5k", "rp_prediction")
...
| 3 | 11개 파이프라인 테이블 + 5개 앱 테이블 + 1개 뷰 존재 | ✅ |
| 4 | `activity_summaries` PRAGMA table_info로 38 컬럼 (id 포함) 확인 | ✅ |
...
| 파일 | 구분 | 설명 |
|------|------|------|
| `src/db_setup.py` | 수정 | v0.3 스키마 재작성, SCHEMA_VERSION=10, 11 pipeline + 5 app tables + 1 view |
