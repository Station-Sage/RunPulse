...
activity_summaries에 넣을지 metric_store에 넣을지의 기준은 "UI에서 자주 쓰이는가"가 아니라
**"이 값의 본질이 무엇인가"**입니다.

**기본 데이터 (activity_summaries)**: 센서가 직접 측정했거나, 측정값에서 단순 산술로
파생된 값. 알고리즘/모델 없이 계산 가능. 동일 입력이면 누가 계산해도 같은 값.
- 예: distance_m, duration_sec, avg_hr, avg_pace_sec_km (= 1000/avg_speed)

**메트릭 (metric_store)**: 알고리즘/모델이 적용된 계산 결과. 동일 활동에 대해
소스/알고리즘에 따라 다른 값이 나올 수 있음. provider로 출처를 구분.
- 예: training_load, normalized_power, calories, trimp, vo2max

이 구분에 따라 activity_summaries에는 순수 측정값과 메타데이터만 남기고,
모든 알고리즘 산출물은 metric_store에 저장합니다.

### 저장 계층 — 5 Layer

```
Layer 0: source_payloads        ← 외부 API 응답 원문 (절대 삭제 안 함)
Layer 1: activity_summaries     ← 통합 활동 요약 (두꺼운 core, ~38컬럼)
...
CREATE INDEX idx_sp_source_entity ON source_payloads(source, entity_type, entity_id);
CREATE INDEX idx_sp_activity ON source_payloads(activity_id);
CREATE INDEX idx_sp_entity_date ON source_payloads(entity_date);
```
...
#### `activity_summaries` — 통합 활동 요약 (~38컬럼)
...
```
**38컬럼** (id 포함). 순수 측정값과 메타데이터만 포함합니다.

이전 설계에서 여기 있던 training_load, suffer_score, training_effect_aerobic,
training_effect_anaerobic, normalized_power, calories는 알고리즘 산출물이므로
metric_store로 이동했습니다. 이 값들은 소스마다 다른 알고리즘으로 계산하며,
RunPulse도 자체 계산할 수 있으므로 metric_store의 provider 체계가 적합합니다.
...
> 컬럼 선정 상세 기준 및 개별 컬럼 판단 근거는 phase-1.md "1-1. 컬럼 포함 기준" 참조.
...
CREATE INDEX idx_df_date ON daily_fitness(date);
CREATE INDEX idx_sync_jobs_source ON sync_jobs(source, created_at);
CREATE INDEX idx_session_outcomes_date ON session_outcomes(date DESC);
...
CREATE INDEX idx_ms_scope_category ON metric_store(scope_type, scope_id, category);
...
CREATE INDEX idx_streams_activity ON activity_streams(activity_id, source);
CREATE INDEX idx_laps_activity ON activity_laps(activity_id);
CREATE INDEX idx_best_efforts_activity ON activity_best_efforts(activity_id);
...
### 전체 테이블 목록 (파이프라인 11 + 앱 5 = 16개 테이블 + 1 뷰)

| Layer | 테이블 | 역할 | 컬럼 수 (id 포함) | 예상 행 수 |
|-------|--------|------|-------------|-----------|
| 0 | `source_payloads` | 외부 API 원문 보존 | 11 | ~3,000 |
| 1 | `activity_summaries` | 통합 활동 요약 (측정값+메타) | 38 | ~600 |
| 1 | `daily_wellness` | 일별 웰니스 요약 | 16 | ~1,500 |
| 1 | `daily_fitness` | 일별 피트니스 모델 | 10 | ~1,500 |
| 2 | `metric_store` | 모든 메트릭 통합 저장소 | 16 | ~55,000 |
| 3 | `activity_streams` | 시계열 GPS/HR/Pace | 15 | ~500,000 |
| 3 | `activity_laps` | 랩/스플릿 | 17 | ~5,000 |
| 3 | `activity_best_efforts` | 베스트 에포트 | 10 | ~2,000 |
| 4 | `gear` | 장비 | 11 | ~20 |
| 4 | `weather_cache` | 날씨 캐시 | 15 | ~1,000 |
| 4 | `sync_jobs` | 동기화 작업 | 13 | ~200 |
...
16개 테이블 + 1 뷰 (파이프라인 11 + 앱 5).


기존 v2의 35+ 테이블에서 **11개 파이프라인 + 5개 앱 = 16개 테이블 + 1 뷰**로 축소했습니다. ETL 문서의 Phase 4에서 계획했던 13개 신규 Garmin 테이블(sleep_data, training_readiness, race_predictions 등)은 전부 `metric_store`의 `scope_type='daily'`로 흡수됩니다.
...
class GarminExtractor:
    ...
    def extract_activity_metrics(self, raw: dict, detail: dict = None) -> list[dict]:
        ...
        _add("intensity_mins_vigorous", raw.get("vigorousIntensityMinutes"),
             category="training_load")
                ...
                # ── activity_summaries에서 이동한 메트릭 ──
        _add("training_load", raw.get("activityTrainingLoad"),
             category="training_load", raw_name="activityTrainingLoad")
        _add("training_effect_aerobic", raw.get("aerobicTrainingEffect"),
             category="training_load", raw_name="aerobicTrainingEffect")
        _add("training_effect_anaerobic", raw.get("anaerobicTrainingEffect"),
             category="training_load", raw_name="anaerobicTrainingEffect")
        _add("normalized_power", raw.get("normPower"),
             category="power", raw_name="normPower")
        _add("calories", raw.get("calories"),
             category="energy", raw_name="calories")
    ...
    def extract_wellness_metrics(self, **raw_payloads) -> list[dict]:
        ...
        if rp:
            _add("race_pred_5k", rp.get("raceTime5K"), category="prediction")
            _add("race_pred_10k", rp.get("raceTime10K"), category="prediction")
            _add("race_pred_half", rp.get("raceTimeHalf"), category="prediction")
            _add("race_pred_marathon", rp.get("raceTimeMarathon"), category="prediction")
        
        return metrics

    def get_activity_with_metrics(conn, activity_id, metric_names=None):
    """활동 기본 정보 + 선택된 메트릭을 합쳐서 반환 (UI용)"""
    # 1. activity_summaries에서 기본 정보
    activity = dict(conn.execute(
        "SELECT * FROM activity_summaries WHERE id = ?", [activity_id]
    ).fetchone())

    # 2. metric_store에서 is_primary=1인 메트릭
    sql = """
        SELECT metric_name, numeric_value, text_value, json_value, provider
        FROM metric_store
        WHERE scope_type = 'activity' AND scope_id = ? AND is_primary = 1
    """
...
### `activity_summaries`와 `metric_store`의 역할 분리

**원칙: 기본 데이터와 메트릭은 저장소가 다르다.**

- `activity_summaries`: 센서 측정값과 메타데이터. 한 활동에 대해 소스당 1행.
- `metric_store`: 모든 알고리즘 산출물. 한 메트릭에 대해 provider당 1행.

이 두 저장소 사이에 중복은 없습니다. training_load가 activity_summaries에도 있고
metric_store에도 있는 상황은 발생하지 않습니다. training_load는 metric_store에만
존재하며, provider=garmin, provider=intervals, provider=runpulse:formula_v1 등으로
구분됩니다.

**Extractor 출력 구조:**

- `extract_activity_core()` → activity_summaries (측정값만)
- `extract_activity_metrics()` → metric_store (알고리즘 산출물 전부)

Garmin의 training_load, Training Effect, calories 등은 `extract_activity_metrics()`에서
metric_store로 저장됩니다. 이전 설계에서 activity_summaries에 있던 이 값들은
소스마다 다른 알고리즘으로 계산되므로, metric_store의 provider 체계로 관리하는 것이
설계 원칙(P3: 같은 메트릭, 다른 provider)에 부합합니다.
...
METRIC_CATEGORIES = {
    ...
    "rp_endurance":      "RunPulse 내구성",
    "rp_recovery":       "RunPulse 회복 분석",
    "rp_distribution":   "RunPulse 강도 분포",
    "rp_classification": "RunPulse 운동 분류",
}
...
- **`rp_` prefix**: RunPulse가 자체 알고리즘으로 계산한 메트릭에만 사용합니다.
  외부 소스(Garmin, Strava, Intervals, Runalyze)가 제공한 메트릭에는 사용하지 않습니다.
- **소스 메트릭 카테고리**: 외부 소스가 제공한 메트릭의 의미적 분류입니다.
  training_load, efficiency, fitness 등.
- **같은 질문에 답하는 메트릭은 같은 metric_name을 사용합니다** (P3).
  Garmin과 RunPulse가 모두 "5K 예측 기록"을 계산한다면, 둘 다 `race_pred_5k`로
  저장하고 provider로 구분합니다.
...
시맨틱 그룹은 **"같은 주제, 다른 질문"의 메트릭을 UI에서 함께 보여줄 때** 사용합니다.

- **Type A (같은 질문, 다른 provider)**: 시맨틱 그룹이 아닙니다.
  metric_store에서 `WHERE metric_name=? GROUP BY provider`로 처리합니다.
  예: trimp(intervals) vs trimp(runpulse) — 같은 metric_name이므로 그룹 불필요.

- **Type B (같은 주제, 다른 질문)**: 시맨틱 그룹의 대상입니다.
  예: training_load(부하 점수) vs suffer_score(고통 지수) vs hrss(심박 스트레스)
  — 모두 "운동 강도"라는 주제이지만 답하는 질문과 단위가 다릅니다.

시맨틱 그룹의 member는 metric_name만 지정합니다. provider는 지정하지 않습니다.
각 metric_name 내에서 어떤 provider를 보여줄지는 is_primary가 결정합니다.

### 시맨틱 그룹 목록 (13개)

| 그룹 | 표시명 | members | 전략 |
|------|--------|---------|------|
| training_load | 훈련 부하 | training_load, suffer_score, hrss, wlei, rtti | show_all |
| trimp | TRIMP | trimp | provider_compare |
| race_prediction | 레이스 예측 | race_pred_5k, race_pred_10k, race_pred_half, race_pred_marathon, rri, marathon_shape | show_all |
| vdot | VDOT | runpulse_vdot, vdot_adj, vo2max_activity, effective_vo2max | prefer_runpulse |
| vo2max | VO2Max | runpulse_vdot, vo2max_activity, effective_vo2max | prefer_runpulse |
| readiness | 준비도 | crs, utrs, training_readiness_score | prefer_runpulse |
| recovery | 회복 | body_battery_high, body_battery_low, rmr | show_all |
| decoupling | 디커플링 | aerobic_decoupling_rp, aerobic_decoupling | provider_compare |
| running_efficiency | 러닝 효율 | rec, efficiency_factor_rp, efficiency_factor | prefer_runpulse |
| threshold_power | 역치 파워 | critical_power, eftp, icu_ftp | prefer_runpulse |
| relative_effort | 상대적 노력 | relative_effort, suffer_score, training_load_score | show_all |
| training_trend | 훈련 추세 | teroi, tpdi, adti | show_all |
| seasonal_performance | 시즌 퍼포먼스 | sapi, fearp | show_all |

**전략 설명:**
- `show_all`: 모든 member를 나란히 표시 (각각 is_primary 값 사용)
- `prefer_runpulse`: RunPulse 계산값 강조, 소스값 보조 표시
- `provider_compare`: 같은 metric_name의 provider별 값 비교 표시
...
# (avg_pace_sec_km은 NULL, avg_hr/duration은 채움. calories는 metric_store)
...
**같은 질문 = 같은 metric_name (P3 적용)**

"이 활동의 TRIMP은?" → `trimp` (provider: intervals, runpulse:formula_v1)
"5K 예상 기록은?" → `race_pred_5k` (provider: garmin, runalyze, runpulse:formula_v1)
"Normalized Power는?" → `normalized_power` (provider: garmin, strava, intervals)

**다른 질문 = 다른 metric_name (P2 적용)**

"이 활동의 Training Load 점수는?" → `training_load`
"이 활동의 TRIMP은?" → `trimp`
"이 활동의 Suffer Score는?" → `suffer_score`
(셋 다 "운동 강도" 주제이지만 계산 방법과 의미가 근본적으로 다름)
...
METRIC_REGISTRY = {
    ...
    # ── Training Load ──
    "trimp": MetricDef("trimp", "rp_load", "AU", "TRIMPexp (Banister)",
        aliases=["icu_trimp"]),
    "hrss": MetricDef("hrss", "rp_load", "점", "HR Stress Score",
        aliases=["icu_hrss"]),
    ...
    "race_pred_5k": MetricDef("race_pred_5k", "prediction", "sec", "5K 예측",
        aliases=["raceTime5K", "race_pred_5k_sec", "darp_5k"]),
    "race_pred_10k": MetricDef("race_pred_10k", "prediction", "sec", "10K 예측",
        aliases=["raceTime10K", "race_pred_10k_sec", "darp_10k"]),
    "race_pred_half": MetricDef("race_pred_half", "prediction", "sec", "하프마라톤 예측",
        aliases=["raceTimeHalf", "race_pred_half_sec", "darp_half"]),
    "race_pred_marathon": MetricDef("race_pred_marathon", "prediction", "sec", "마라톤 예측",
        aliases=["raceTimeMarathon", "race_pred_marathon_sec", "darp_marathon"]),
    ...
    "rmr": MetricDef("rmr", "rp_recovery", None, "러너 성숙도 레이더",
        value_type="json"),
...
다음 메트릭은 Garmin API에서 제공하지만 아직 extractor에 구현되지 않았습니다.
구현 시 이 목록에서 위의 정규 사전으로 이동합니다.

- `skin_temp`: 피부 온도
- `fitness_age`: 피트니스 나이
- `hill_score`: 힐 스코어
- `heat_acclimation_pct`: 더위 적응도
- `altitude_acclimation`: 고도 적응도
- `training_status`: 훈련 상태
- `training_status_feedback`: 훈련 상태 피드백
- `avg_sleep_stress`: 수면중 평균 스트레스
- `avg_spo2_sleep`: 수면중 평균 SpO2
- `training_readiness_acute_load_factor`: 급성부하 요인
- `race_shape`: 레이스 준비도

### Phase 1 — 기반 (1주)

1. `db_setup.py` 전면 재작성 (11개 파이프라인 + 5개 앱 테이블)
...
### Phase 4 — Metrics Engine 구현 + 포팅 + API 정비 (메트릭 정합성 수정중)
...
18. activity_summaries에서 메트릭 6개 제거, metric_store로 이동
19. 4개 extractor 수정 (extract_activity_core → extract_activity_metrics 이동)
20. db_helpers.py upsert 함수 수정
21. src/services/ 서비스 레이어 구현
22. src/web/ 뷰 수정 (metric_store 조회 추가)
23. 테스트 전체 수정 및 통과
