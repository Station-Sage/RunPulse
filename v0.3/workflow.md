...
5. BACKLOG.md 해당 항목 [x] 체크 및 DONE으로 이동
...
5. `python scripts/check_docs.py` 실행 및 불일치 수정 (Phase 3 이후)
...
`scripts/check_docs.py` 생성. Claude Code가 작업 완료 후 실행.

**검사 항목:**
- BACKLOG.md의 NOW 항목 수 (3개 이하인지)
- 각 GUIDE.md에 등록된 파일 vs 실제 존재 파일 불일치
- 300줄 초과 파일 목록 출력
- pytest 수집 수 vs BACKLOG.md 기재 수 비교
...
1. scripts/check_docs.py 생성
2. CLAUDE.md 완료 규칙에 스크립트 실행 추가
3. 1~2주 운영하며 효과 확인
