...
[비전-코드 공식 차이] (D-V2-08b 기준, PDF 우선 채택)
  - PDF 원본 (채택): ACWR(0.4) + Monotony(0.2) + Spike/LSI주간(0.3) + Asymmetry/GCT(0.1)
    → 생체역학(GCT 비대칭) 포함, 훈련 단조로움(Monotony) 반영
  - Claude 연구: ACWR(0.4) + LSI(0.25) + 연속훈련일(0.2) + 피로누적/CTL-TSB(0.15)
    → GCT 불필요, 피로 상태 중심. 구성요소 자체가 다름
  - 핵심 차이:
    · 2번 요소: PDF=Monotony(반복성) vs Claude=LSI(부하 급증)
    · 3번 요소: PDF=Spike/LSI(주간 부하비) vs Claude=연속훈련일수
    · 4번 요소: PDF=GCT 비대칭(생체역학) vs Claude=CTL-TSB(피로 누적)
