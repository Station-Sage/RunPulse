...
def render_pattern_insights(data_14d: list[dict], baseline: dict,
                            ai_override: str | None = None) -> str:
    """패턴 인사이트 — ai_override(탭 통합 AI) 우선, 규칙 기반 fallback."""
    if len(data_14d) < 3:
        return ""
    ...
    if ai_override:
        return (
            ...
            f"<div style='font-size:0.83rem;color:var(--secondary);line-height:1.7;'>{ai_override}</div></div>"
        )
