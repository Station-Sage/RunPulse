...
_METRIC_META: dict[str, tuple[str, object]] = {
    ...
    "MarathonShape":  ("레이스 준비도 (Race Shape) — 볼륨+최장거리+장거리빈도+일관성+페이스품질 5요소 (0~100%).",
                       lambda v: ("레이스 준비 미흡" if v < 50 else "기본 준비됨" if v < 80 else "레이스 준비 완료", "var(--red)" if v < 50 else "var(--orange)" if v < 80 else "var(--green)")),
